(function ($) {
    ...
    if ($("#wish-form").length) {
        ...
        function autoloadWish(){
            var sheet_id = '1s1JqzXgeq-cdOEu_o82FzI65ZtiziLPEwENUa41DAbM';
            var search = 'select B where B contains "name"';
            $.ajax({
                url : `https://docs.google.com/spreadsheets/d/${sheet_id}/gviz/tq?tq=${search}`,
                cache: false,
                ...
                if (data.status == 200){
                    try{
                        var response = JSON.parse(/setResponse\((.*)\)\;/g.exec(data.responseText)[1]);
                        var result = response.table.rows.map(el => {
                            return  convertData(JSON.parse(el.c[0].v))
                        });
                        $('.wish-box').html(result.map((el, index) => {
                            return '<div class="wish-box-item '+(index % 2 ? 'bg' : '')+'"><strong>' + el.name.replace(/&/g, "&amp;").replace(/>/g, "&gt;").replace(/</g, "&lt;").replace(/"/g, "&quot;") + '</strong><p>' + el.content.replace(/&/g, "&amp;").replace(/>/g, "&gt;").replace(/</g, "&lt;").replace(/"/g, "&quot;") + '</p></div>'
                        }).join(''))
        ...
        $("#wish-form").validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({ 
                    url: `https://docs.google.com/forms/d/e/1FAIpQLSdHZHoHVLOk9SerdfajtvLOFHHqoe1AfAiA46XQqvGh683yqg/formResponse`, 
                    type: "POST", 
                    crossDomain: !0, 
                    dataType: "xml", 
                    data: {
                        "entry.1178347282" : JSON.stringify($(form).serializeArray())
                    }, 
                    statusCode: { 
                        0: function (t) { 
                            $("#loader").hide();
                            ...
                            $("#success").html("Cảm ơn bạn!").slideDown("slow");
                            setTimeout(function () {
